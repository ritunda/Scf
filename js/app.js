...
async function verifyInviteCode() {
  ...
  if (!code) { errorEl.innerText = "Enter an invite code"; return; }
  ...
  try {
    ...
    if (error) { errorEl.innerText = "Error: " + error.message; return; }
    if (!data) { errorEl.innerText = "Invalid invite code"; return; }
    if (data.used) { errorEl.innerText = "This code has already been used"; return; }
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="dropdown">
            ...
            <div class="dropdown-content">
              ${post.user_id === currentUser.id ? `<a href="#" onclick="editPost('${post.id}', '${escapeHtml(post.content).replace(/'/g, "\\'")}', ${post.is_anonymous}, '${post.specialty_tag || ''}')">Edit</a>` : ''}
...
async function votePost(postId, type) {
  const { error } = await supabase.from('votes').upsert(
    { post_id: postId, user_id: currentUser.id, vote_type: type },
    ...
    <div id="comment-${c.id}" class="text-sm bg-gray-50 rounded p-2" data-post-id="${postId}">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <strong>${c.profiles?.full_name || 'Alumni'}</strong>
      ...
      ${profiles.map(p => `
        <div class="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition">
          <div class="flex items-center gap-3">
            <div class="w-12 h-12 bg-gray-200 rounded-full flex items-center justify-center text-gray-600 text-xl">${(p.full_name || 'A').charAt(0)}</div>
            <div>
              <div class="font-semibold">${escapeHtml(p.full_name || 'Alumni')}</div>
              <div class="text-sm text-gray-500">${escapeHtml(p.specialty || 'General')}</div>
            </div>
          </div>
          <div class="mt-2 text-sm text-gray-600">Class of ${p.graduation_year || '?'}</div>
          <div class="text-sm text-gray-600">${escapeHtml(p.workplace || '')}</div>
...
async function renderJobs() {
  ...
  document.getElementById('appContent').innerHTML = `
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold">Job Board</h2>
      ${currentProfile?.is_verified ? `<button onclick="openJobModal()" class="btn-primary">+ Post a Job</button>` : ''}
    </div>
    <div class="space-y-4">
      ${jobs.map(job => `
        <div class="bg-white rounded-xl border border-gray-200 p-4">
          <h3 class="text-xl font-bold">${escapeHtml(job.title)}</h3>
          <div class="text-[#0a66c2]">${escapeHtml(job.employer)}</div>
          <div class="text-sm text-gray-500">${escapeHtml(job.location || 'Remote')}</div>
          <p class="mt-2 text-gray-700">${escapeHtml(job.description)}</p>
          <div class="mt-2 text-xs text-gray-400">Posted by ${escapeHtml(job.profiles?.full_name || 'Alumni')}</div>
        </div>
      `).join('')}
    </div>
  `;
...
async function renderMentorship() {
  ...
  document.getElementById('appContent').innerHTML = `
    ...
    <div class="grid md:grid-cols-2 gap-6 mb-6">
      <div class="bg-white rounded-xl border border-gray-200 p-4">
        <h3 class="font-semibold text-lg">Incoming Requests</h3>
        ${incoming.length ? incoming.map(c => `
          <div class="flex justify-between py-2">
            <span>${escapeHtml(c.mentee?.full_name || 'Alumni')}</span>
            <div>
              <button onclick="updateMentorship('${c.id}','accept')" class="btn-primary text-xs py-1 px-3">Accept</button>
              <button onclick="updateMentorship('${c.id}','reject')" class="btn-outline text-xs py-1 px-3 ml-2">Reject</button>
            </div>
          </div>
        `).join('') : '<p class="text-gray-500">None</p>'}
      </div>
      <div class="bg-white rounded-xl border border-gray-200 p-4">
        <h3 class="font-semibold text-lg">Active Mentorships</h3>
        ${active.length ? active.map(c => `
          <div class="py-1">${c.mentor_id === currentUser.id ? `Mentee: ${escapeHtml(c.mentee?.full_name || '')}` : `Mentor: ${escapeHtml(c.mentor?.full_name || '')}`}</div>
        `).join('') : '<p class="text-gray-500">None</p>'}
      </div>
    </div>
  `;
...
async function updateMentorship(connId, action) {
...
async function renderEvents() {
  ...
  document.getElementById('appContent').innerHTML = `
    <div class="flex justify-between items-center mb-6">
      ...
      ${currentProfile?.is_verified ? `<button onclick="openEventModal()" class="btn-primary">+ Create Event</button>` : ''}
    </div>
    <div class="grid md:grid-cols-2 gap-4">
      ${events.map(ev => `
        <div class="bg-white rounded-xl border border-gray-200 p-4">
          <h3 class="text-xl font-bold">${escapeHtml(ev.title)}</h3>
          <div class="text-[#0a66c2] text-sm">${new Date(ev.start_time).toLocaleString()}</div>
          <p class="mt-2">${escapeHtml(ev.description || '')}</p>
          ${ev.virtual_link ? `<a href="${ev.virtual_link}" target="_blank" class="text-[#0a66c2]">Join online</a>` : `<div>📍 ${escapeHtml(ev.location || '')}</div>`}
        </div>
      `).join('')}
    </div>
  `;
  ...
  document.getElementById('appContent').innerHTML = `
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div class="px-6 pb-6 -mt-12">
        <div class="w-24 h-24 bg-white rounded-full border-4 border-white shadow flex items-center justify-center text-4xl font-bold">
          ${(currentProfile.full_name || 'A').charAt(0)}
        </div>
        <h1 class="text-2xl font-bold mt-2">${escapeHtml(currentProfile.full_name || 'Alumni')}</h1>
        <div class="text-[#0a66c2]">${escapeHtml(currentProfile.specialty || 'General')}</div>
        <div class="text-gray-500">Class of ${currentProfile.graduation_year || '?'} • ${escapeHtml(currentProfile.workplace || '')}</div>
        <div class="mt-2 flex gap-4 text-sm"><span>👥 Followers: ${followers || 0}</span><span>👤 Following: ${following || 0}</span></div>
        <div class="mt-4">${escapeHtml(currentProfile.bio || 'No bio yet.')}</div>
      </div>
    </div>
    <div class="mt-6 bg-white rounded-xl border border-gray-200 p-4">
      <h3 class="font-semibold text-lg mb-2">Skills & Endorsements</h3>
      <div class="flex flex-wrap gap-2">
        ${endorsements?.map(e => `<span class="bg-gray-100 px-3 py-1 rounded-full text-sm">${escapeHtml(e.skill)}</span>`).join('') || 'None yet'}
      </div>
    </div>
  `;
}
    ...
    <div class="space-y-4">
      ${Object.values(conversations).map(conv => `
        <div class="bg-white rounded-xl border border-gray-200 p-4 flex justify-between items-center">
          <div>
            <strong>${escapeHtml(conv.otherName || 'Alumni')}</strong>
            <div class="text-sm text-gray-500">${escapeHtml(conv.content.substring(0, 50))}</div>
          </div>
  ...
  document.getElementById('appContent').innerHTML = `
    <div class="space-y-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div class="bg-white p-4 rounded-xl shadow-sm text-center"><div class="text-3xl font-bold">${userCount || 0}</div><div class="text-gray-500">Users</div></div>
        <div class="bg-white p-4 rounded-xl shadow-sm text-center"><div class="text-3xl font-bold">${postCount || 0}</div><div class="text-gray-500">Posts</div></div>
        <div class="bg-white p-4 rounded-xl shadow-sm text-center"><div class="text-3xl font-bold">${commentCount || 0}</div><div class="text-gray-500">Comments</div></div>
      </div>
      <div class="bg-white rounded-xl shadow-sm p-4">
        <h3 class="text-xl font-bold mb-4">Invite Codes</h3>
        <div class="flex gap-2 mb-4">
          <input id="newInviteCode" placeholder="New invite code" class="flex-1 p-2 border rounded">
          <button onclick="createInviteCode()" class="btn-primary">Create</button>
        </div>
        <div class="space-y-1 text-sm">
          ${(inviteCodes || []).map(c => `
            <div class="flex justify-between items-center border-b py-1">
              <code>${escapeHtml(c.code)}</code>
              <span class="${c.used ? 'text-red-500' : 'text-green-600'}">${c.used ? 'Used' : 'Available'}</span>
            </div>
          `).join('')}
        </div>
      </div>
...
async function loadView(view) {
  ...
  try {
    ...
    container.innerHTML = `<div class="bg-red-50 text-red-700 p-4 rounded-xl">Error: ${escapeHtml(e.message)}</div>`;
  }
...
async function initApp(user) {
  ...
  document.getElementById('userNameDisplay').innerText = (profile.full_name || 'Alumni').split(' ')[0];
  document.querySelector('#profileBtn div').innerText = (profile.full_name || 'A').charAt(0);
...
waitForSupabase()
  .then(() => {
    console.log("✅ App ready — enabling UI");

    const btn = document.getElementById('verifyBtn');
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = 'Verify';
      btn.onclick = () => window.verifyInviteCode();
    }
